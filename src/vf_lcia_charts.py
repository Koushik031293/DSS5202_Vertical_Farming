#!/usr/bin/env python3
# vf_lcia_charts.py
# Generate LCIA charts and compile them into a single Markdown report (+ optional PDF).
...
def chart_totals_bar(totals_df, outpath, fmt="png", dpi=160):
    ...
    plt.tight_layout(); plt.savefig(f"{outpath}/totals_bar.{fmt}", dpi=dpi); plt.close()
...
def chart_by_stage_stacked(by_stage_df, outpath, fmt="png", dpi=160):
    ...
    plt.tight_layout(); plt.savefig(f"{outpath}/by_stage_stacked_bar.{fmt}", dpi=dpi); plt.close()
...
def chart_gwp_stage_pie_or_bar(by_stage_df, outpath, fmt="png", dpi=160,
                               dominance_threshold=0.9, min_slice_pct=0.02, legend_pie=True):
    df = by_stage_df.copy()
    if "stage" not in df.columns or "GWP100" not in df.columns: return
    s = df.set_index("stage")["GWP100"]; s = s[s > 0]
    if s.empty: return
    s = s.sort_values(ascending=False); total = float(s.sum())
    if s.iloc[0] >= dominance_threshold * total:
        plt.figure(figsize=(7,4))
        s.sort_values().plot(kind="barh")
        plt.xlabel("GWP100 (kg CO₂e per kg)")
        plt.title("GWP100 by Stage (per kg)")
        plt.tight_layout(); plt.savefig(f"{outpath}/gwp_stage_bar.{fmt}", dpi=dpi); plt.close(); return
    ...
    wedges, texts, autotexts = ax.pie(values, explode=explode, startangle=90,
                                      autopct=_autopct, pctdistance=0.8,
    ...
    plt.tight_layout(); plt.savefig(f"{outpath}/gwp_stage_pie.{fmt}", dpi=dpi, bbox_inches="tight"); plt.close(fig)

def chart_normalized_radar(norm_totals_df, outpath, fmt="png", dpi=160):
    df = norm_totals_df.copy()
    if df is None or df.empty: return
    if "category" not in df.columns or "normalized_perkg" not in df.columns: return
...
def chart_top_flows(lcia_rows_df, outpath, fmt="png", dpi=160, topn=10, category="GWP100"):
    ...
    if perkg_col not in df.columns or "flow_name" not in df.columns: return
...
def chart_per_category_stage_bars(by_stage_df, outpath, fmt="png", dpi=160, logy=False):
    ...
    for cat in [c for c in CATS if c in df.columns]:
        ...
        plt.ylabel(cat); plt.title(f"{cat} by Stage (per kg)")
        plt.tight_layout(); plt.savefig(f"{outpath}/by_stage_{cat}_bar.{fmt}", dpi=dpi); plt.close()
...
def chart_normalized_by_stage(by_stage_df, outpath, fmt="png", dpi=160):
    ...
    plt.ylim(0,1); plt.ylabel("Relative impact (0–1 within category)")
    plt.title("Normalized LCIA by Stage (per category scaled to max=1)")
    plt.tight_layout(); plt.savefig(f"{outpath}/normalized_by_stage_stacked.{fmt}", dpi=dpi); plt.close()
...
def build_markdown(outdir, fmt, md_path, title="Vertical Farming LCIA – Charts"):
    ...
    lines = [f"# {title}", "", 
             ...
             "## Contents"]
    toc_items = [("totals","Total Impact per Category"),
    ...
    lines += [f"- [{text}](#{anchor})" for anchor, text in toc_items] + [""]

    # 1
    lines.append("## Total Impact per Category {#totals}")
    rel = _rel(md_path, expect[0][1]) if _exists(expect[0][1]) else None
    lines.append(f"![Total impact per category]({rel})" if rel else "_Chart not generated._"); lines.append("")
    # 2
    lines.append("## LCIA by Stage (Stacked) {#stacked}")
    rel = _rel(md_path, expect[1][1]) if _exists(expect[1][1]) else None
    lines.append(f"![LCIA by Stage (stacked)]({rel})" if rel else "_Chart not generated._"); lines.append("")
    # 3
    lines.append("## GWP100 by Stage (Pie/Bar) {#gwp}")
    gwp_path = expect[2][1]
    rel = _rel(md_path, gwp_path) if _exists(gwp_path) else None
    lines.append(f"![GWP100 by stage]({rel})" if rel else "_Chart not generated._"); lines.append("")
    # 4
    lines.append("## Normalized Radar (if available) {#radar}")
    rel = _rel(md_path, expect[3][1]) if _exists(expect[3][1]) else None
    lines.append(f"![Normalized radar]({rel})" if rel else "_Chart not generated or normalization sheet missing._"); lines.append("")
    # 5
    lines.append("## Top Flows by GWP100 {#topflows}")
    rel = _rel(md_path, expect[4][1]) if _exists(expect[4][1]) else None
    lines.append(f"![Top flows by GWP100]({rel})" if rel else "_Chart not generated._"); lines.append("")
    # 6
    if _exists(expect[5][1]):
        lines.append("## Normalized by Stage (Stacked 0–1) {#normstage}")
        rel = _rel(md_path, expect[5][1]); lines.append(f"![Normalized by Stage (stacked 0–1)]({rel})"); lines.append("")
    # 7
    if per_cat:
        lines += ["## Per-Category Stage Bars {#percat}",
        ...
        for p in sorted(per_cat, key=cat_key):
            m = re.search(r"by_stage_(.+?)_bar\.", os.path.basename(p))
            cat = m.group(1) if m else "Category"
            rel = _rel(md_path, p)
            lines += [f"### {cat}", f"![{cat} by Stage]({rel})", ""]
...
def convert_md_to_pdf(md_file, out_pdf=None, pdf_engine=None):
    ...
    try:
        pypandoc.convert_text(open(md_file, encoding="utf-8").read(),
                              "pdf", format="md",
                              outputfile=out_pdf, extra_args=extra)
...
def images_to_pdf(img_dir, out_pdf):
    ...
    if not imgs:
        print("⚠️ No images to convert."); return
    pages = []
    for p in imgs:
        im = Image.open(p)
...
# ============ CLI ============
def main():
    ap = argparse.ArgumentParser(description="Generate LCIA charts (and Markdown/PDF report).")
    ...
    ap.add_argument("--dominance-threshold", type=float, default=0.9, help="Max share for pie; else bar")
    ap.add_argument("--min-slice-pct", type=float, default=0.02, help="Group small pie slices into 'Other'")
    ap.add_argument("--legend-pie", action="store_true", help="Use legend instead of labels for pie")
    ...
    # Markdown + PDF
    ap.add_argument("--write-md", action="store_true", help="Write Markdown report embedding charts")
    ap.add_argument("--md-file", default="LCIA_charts_report.md", help="Markdown file path")
    ap.add_argument("--md-title", default="Vertical Farming LCIA – Charts", help="Markdown title")
    ap.add_argument("--make-pdf", action="store_true", help="Convert Markdown report to PDF (requires pandoc engine)")
    ap.add_argument("--pdf-engine", choices=["pdflatex","xelatex","wkhtmltopdf"], help="Force a specific PDF engine")
    ap.add_argument("--images-only-pdf", action="store_true", help="Stitch chart images into a PDF (no pandoc)")
    ...
    # Images-only PDF (fallback or extra)
    if args.images_only_pdf:
        images_to_pdf(args.outdir, os.path.join(args.outdir, "LCIA_charts_images.pdf"))
